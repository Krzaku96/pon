...
if __name__ == "__main__":
    ...
    y_train: np.ndarray = data_set.import_columns_train(
        np.array(['quality']))
    y_test: np.ndarray = data_set.import_columns_test(
        np.array(['quality']))

    NN = MLPClassifier(solver='lbfgs', alpha=1e-2,
                       hidden_layer_sizes=(150, 11),
